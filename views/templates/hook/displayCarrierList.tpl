...
<div id="delivery-options">
    {if !empty($mymodcarrier_relay_point)}
        ...
        {foreach from=$mymodcarrier_relay_point key=id_relay_point item=relay_point}
            <p>
                <input type="radio" name="relay_point"
                       class="mymodcarrier_relay_point" data-token="{$relay_point.token}" 
                       value="{$relay_point.name|escape:'htmlall'|urlencode}:%20{$relay_point.address|escape:'htmlall'|urlencode}" {if $id_relay_point eq 0}checked="checked"{/if} />
